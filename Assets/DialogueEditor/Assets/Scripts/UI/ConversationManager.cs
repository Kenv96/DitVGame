...
namespace DialogueEditor
{
    public class ConversationManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            Player = GameObject.Find("Player").gameObject.GetComponent<PlayerMove>();
            PlayerCamera = GameObject.Find("ExploreCam").gameObject.GetComponent<CinemachineFreeLook>();
        ...
        private void CheckQuestLog()
        {
            List<QuestScriptObj> questLog =GlobalQuestLog.GetInstance().GQL;
            foreach(Parameter parameter in m_conversation.Parameters)
            {
                int questcode;
                if (parameter.ParameterName.ToString().Length == 1)
                {
                    questcode = int.Parse(parameter.ParameterName);
                    foreach(QuestScriptObj quest in questLog)
                    {
                        if(quest.GetQuestCode() == questcode)
                        {
                            SetInt(questcode.ToString(), quest.questStepCount);
                            SetBool(questcode.ToString() + "a", quest.isActive);
                            SetBool(questcode.ToString() + "c", quest.isComplete);
                        }
                    }
                }               
            }
